...
export const exportMailEventSchema = z.object({
	...
	estimate: z.object({
		remainingForums: z.number().int().nonnegative(),
		remainingForumPageTasks: z.number().int().nonnegative(),
		remainingThreadTasks: z.number().int().nonnegative(),
		...
		basedOn: z.enum(["history", "progress", "blended", "insufficient_data"]),
